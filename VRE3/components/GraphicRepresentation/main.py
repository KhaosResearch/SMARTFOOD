...
def main(fitness_evolution_file: str, fun_file: str, output_folder: str):
    ...
    z = zipfile.ZipFile(f"{output_folder}/output.zip", "w", zipfile.ZIP_DEFLATED)
    ...
    filename = "fitness_evolution.html"
    ...
    if len(objectives) == 3:
        ...
        for i in range(len(objectives)):
            for j in range(i + 1, len(objectives)):
                ## Set pareto front filename
                filename = f"pareto_front_2D_{objectives[i]}-{objectives[j]}.html"
